 ...
 */
function comandos_get_modern_url(string $url, string $format = 'avif'): string {
    ...
    if ($lcp_img_id) {
        ...
        if ($img_src) {
            ...
            foreach ($formats as $fmt) {
                $cand_src = comandos_get_modern_url($img_src, $fmt);
                $path_check = str_replace($base_url, $base_path, strtok($cand_src, '?'));
                if (file_exists($path_check)) {
                    $final_src = $cand_src;
                    if ($img_srcset) {
                        $final_srcset = comandos_get_modern_url($img_srcset, $fmt);
                    }
                    break;
                }
            }
    ...
    if ($logo_id) {
        ...
        if ($logo_src) {
            ...
            foreach (['avif', 'webp'] as $fmt) {
                $cand_logo = comandos_get_modern_url($logo_src, $fmt);

...
Для параллельной тестовой установки на том же VPS можно задать
`COMANDOS_CADDY_BLOCK_ID`, чтобы установщик не перезаписывал основной
управляемый блок Caddy. По умолчанию используется `COMANDOS HERMES`.

Файлы развёртывания отправляются на VPS одним SSH-потоком через архив, чтобы
не создавать серию быстрых SCP-подключений. SSH-шаг повторяется до 5 раз,
если соединение временно закрылось до выполнения команды. Количество попыток
можно изменить через `COMANDOS_SSH_RETRIES`, паузу между попытками — через
`COMANDOS_SSH_RETRY_DELAY_SECONDS`.

Если SSH-вход выполнен не под `root`, установщик использует `sudo` для
системных действий: установка пакетов, создание `REMOTE_APP_USER`, настройка
Caddy и systemd. Архивы создаются без macOS xattrs, чтобы на Linux/VPS не было
предупреждений `LIBARCHIVE.xattr...`.
...
COMANDOS_WORKSPACE_VERSION=2.3.0-comandos.6

...
SSH/SCP-шаги развёртывания повторяются до 5 раз, если соединение временно
закрылось до выполнения команды. Количество попыток можно изменить через
`COMANDOS_SSH_RETRIES`, паузу между попытками — через

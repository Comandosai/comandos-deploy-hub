...
| Обновления | Кнопка обновления успевала перезапустить панель до ответа API; state записывал старую версию из lock | `comandos-hermes.lock` поднят до `2.3.0-comandos.12`; restart delay увеличен до 8 сек и вынесен в `COMANDOS_WORKSPACE_RESTART_DELAY_SECONDS` | На `clawd` кнопка запустила backup и обновила код до `.12`; ошибка соединения воспроизведена и причина найдена | FIXED |

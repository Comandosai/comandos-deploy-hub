...
| QA Playwright: пункт `HermesWorld` на `.38` | FIXED | Экран открывался, но browser console показывала autoplay warning `AudioContext was not allowed to start`. В `.39` звуки и ambient ставятся в очередь и AudioContext создаётся только после пользовательского жеста |

...
| Установка | Чистая параллельная установка на том же VPS могла перезаписать рабочий Caddy-блок `COMANDOS HERMES` | Workspace поднят до `2.3.0-comandos.33`; `deploy.sh` поддерживает `COMANDOS_CADDY_BLOCK_ID`, по умолчанию оставляет старый блок, а QA-контур может использовать отдельный marker | `bash -n` для установочных скриптов OK; Python-проверка Caddy marker сохраняет основной блок и заменяет только QA-блок; `update-system` тесты и `pnpm build` OK | FIXED |
| Установка | При чистой установке SSH на VPS временно закрыл соединение до баннера, и `deploy.sh` падал с `Connection closed by ... port 22` | Workspace поднят до `2.3.0-comandos.33`; SSH-шаг deploy получил ограниченный retry с настраиваемыми `COMANDOS_SSH_RETRIES` и `COMANDOS_SSH_RETRY_DELAY_SECONDS` | Чистая установка `.30` показала повторы на `SSH remote install`; основной сервер не был изменён | FIXED |
| Установка | После всех неудачных SSH-повторов `deploy.sh` мог напечатать локальное «готово», хотя удалённая установка не стартовала | Workspace поднят до `2.3.0-comandos.33`; retry теперь вызывает явный `fail` после последней неудачной попытки и валидирует retry-переменные | Серверная проверка после ложного успеха: QA-каталога нет, QA-порты не слушают, Caddy не изменён; `.31` уже падает честно без ложного успеха | FIXED |
| Установка | Deploy делал серию быстрых подключений `ssh + scp + scp + scp + scp + ssh`, что провоцировало rate-limit sshd на VPS | Workspace поднят до `2.3.0-comandos.33`; payload, env, lock и remote installer упаковываются в один deploy bundle и отправляются одним SSH-потоком | `.32` дошла до remote install одним SSH-потоком, без SCP-серии | FIXED |
| Установка | При входе на VPS под sudo-пользователем новый `REMOTE_APP_USER` не создавался, потому что скрипт требовал root | Workspace поднят до `2.3.0-comandos.33`; создание app-пользователя теперь идёт через `$SUDO useradd` | Чистая `.32` дошла до ошибки `пользователь hermesqa не найден, а текущий пользователь не root`; фикс готовится к повторной проверке `.33` | FIXED |
| Установка | macOS xattrs попадали в deploy-архив и на VPS печатались предупреждения `LIBARCHIVE.xattr.com.apple.provenance` | Workspace поднят до `2.3.0-comandos.33`; tar-архивы создаются с `--no-xattrs` | Локальный tar поддерживает `--no-xattrs`; повторная чистая установка проверит отсутствие предупреждений | FIXED |

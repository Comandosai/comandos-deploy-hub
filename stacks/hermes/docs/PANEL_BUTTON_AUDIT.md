...
| QA Playwright: пункт `HermesWorld` на `.39` | FIXED | Найдены 2 оставшихся autoplay warning из общего `src/lib/sounds.ts`. В `.40` общий звук панели агента/чата тоже ждёт пользовательский жест перед созданием `AudioContext` |

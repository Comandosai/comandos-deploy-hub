...
| Установка | Чистая параллельная установка на том же VPS могла перезаписать рабочий Caddy-блок `COMANDOS HERMES` | Workspace поднят до `2.3.0-comandos.31`; `deploy.sh` поддерживает `COMANDOS_CADDY_BLOCK_ID`, по умолчанию оставляет старый блок, а QA-контур может использовать отдельный marker | `bash -n` для установочных скриптов OK; Python-проверка Caddy marker сохраняет основной блок и заменяет только QA-блок; `update-system` тесты и `pnpm build` OK | FIXED |
| Установка | При чистой установке SSH на VPS временно закрыл соединение до баннера, и `deploy.sh` падал с `Connection closed by ... port 22` | Workspace поднят до `2.3.0-comandos.31`; SSH/SCP-шаги deploy получили ограниченный retry с настраиваемыми `COMANDOS_SSH_RETRIES` и `COMANDOS_SSH_RETRY_DELAY_SECONDS` | Чистая установка `.30` показала повторы на `SSH remote install`; основной сервер не был изменён | FIXED |
| Установка | После всех неудачных SSH-повторов `deploy.sh` мог напечатать локальное «готово», хотя удалённая установка не стартовала | Workspace поднят до `2.3.0-comandos.31`; retry теперь вызывает явный `fail` после последней неудачной попытки и валидирует retry-переменные | Серверная проверка после ложного успеха: QA-каталога нет, QA-порты не слушают, Caddy не изменён; фикс готовится к повторной чистой установке | FIXED |

...
| Терминал | При длинном проходе по меню после `/terminal` снова появились 502 на `/api/terminal-resize`, а в логах — `MaxListenersExceededWarning` | TerminalWorkspace теперь не инициализирует PTY и не resize-ит терминал, пока раздел скрыт; `/api/terminal-stream` снимает listeners и keepalive при `close` и abort | Unit-тест stream cleanup проходит; живая проверка выполняется после установки `.18` | FIXED |

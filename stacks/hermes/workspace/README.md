...
<img src="./public/komandos/logo-mark.png" alt="COMANDOS AI Workspace" width="80" style="border-radius: 16px" />

# COMANDOS AI Workspace

**Командный центр клуба для AI-команды: чат, файлы, память, навыки, задачи и терминал в одном месте.**
...
> Не chat wrapper, а рабочее место оператора: оркестрируйте агентов, просматривайте память, управляйте skills и контролируйте систему из одного интерфейса.

> **COMANDOS AI hard fork.** Built from `outsourc-e/hermes-workspace` and powered by vanilla [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent). Upstream MIT attribution is preserved; visible product experience is COMANDOS AI Workspace.

![COMANDOS AI Workspace](./docs/screenshots/splash.png)
...
- 🎨 **Themes** — COMANDOS Dark and COMANDOS Light
...
```bash
curl -fsSL https://raw.githubusercontent.com/Comandosai/comandos-workspace/main/install.sh | bash
```
...
cd ~/comandos-hermes-workspace && pnpm dev   # terminal 2
```
...
```bash
git clone https://github.com/Comandosai/comandos-workspace.git
cd comandos-workspace
pnpm install
cp .env.example .env

# COMANDOS uses one visible panel by default.
echo 'COMANDOS_SINGLE_PANEL=1' >> .env
...
- `curl http://127.0.0.1:3000/api/local-hermes/status` should report `mode: "single-panel"` and `ok: true` after the workspace starts.

Then start the workspace and complete onboarding. The native Hermes dashboard is optional legacy compatibility.
...
Then restart the gateway and workspace. Hit the workspace from the remote device and the connection probe will use the Tailscale IP instead of localhost.
...
COMANDOS AI Workspace works with any OpenAI-compatible backend. If your backend also exposes Hermes Agent gateway APIs, enhanced features like sessions, memory, skills, and jobs unlock automatically.
...
Point COMANDOS AI Workspace at any backend that supports:
...
### Step 2: Install & Run COMANDOS AI Workspace
...
COMANDOS AI Workspace supports two modes with local models:
...
Download [Atomic Chat](https://atomic.chat/), launch the desktop app, and make sure a model is loaded before starting COMANDOS AI Workspace.
...
Workspace uses the gateway for chat/models and local `~/.hermes` files for config and local skills. The native dashboard is optional legacy compatibility.
...
Workspace is the UI. **Hermes Agent** is the brain. They talk through the gateway HTTP API on localhost (or any reachable network).
...
### Two services, two commands

```bash
hermes gateway run     # terminal 1 · :8642 · chat, models, streaming, jobs
cd ~/comandos-hermes-workspace && pnpm dev   # terminal 2 · :3000 · the UI
```

> **Tip:** `pnpm start:all` starts gateway + COMANDOS Workspace in one shot. Use `pnpm start:legacy-dashboard` only when you intentionally need the old dashboard.

### Windows (PowerShell + WSL) one-command startup

If you use COMANDOS AI Workspace from Windows with the agent running in WSL, use the helper script in this repo:
...
# Default: one visible COMANDOS panel
COMANDOS_SINGLE_PANEL=1
...
| Scenario | Set this |
|---|---|
| Workspace + gateway on the same machine | `HERMES_API_URL=http://127.0.0.1:8642`, `COMANDOS_SINGLE_PANEL=1` |
...
| Already-running `hermes-agent` from upstream installer | Set `HERMES_API_URL`, `COMANDOS_SINGLE_PANEL=1`, and skip the one-liner installer |
| Multiple agent profiles | Profiles live under `~/.hermes/profiles/<name>` |
...
- **Workspace shows "portable mode" / extended APIs missing** — this is acceptable in COMANDOS single-panel mode if chat, config, and local skills work.
...
The Docker setup runs both the **Hermes Agent gateway** and **COMANDOS AI Workspace** together.
...
Deploying COMANDOS AI Workspace to a PaaS or home-lab stack? Pull the image
...
COMANDOS AI Workspace is a **Progressive Web App (PWA)** — install it for the full native app experience with no browser chrome, keyboard shortcuts, and offline support.

### 🖥️ Desktop (macOS / Windows / Linux)

1. Open COMANDOS AI Workspace in **Chrome** or **Edge** at `http://localhost:3000`
2. Click the **install icon** (⊕) in the address bar
3. Click **Install** — COMANDOS AI Workspace opens as a standalone desktop app
...
1. Open COMANDOS AI Workspace in **Safari** on your iPhone
2. Tap the **Share** button (□↑)
3. Scroll down and tap **"Add to Home Screen"**
4. Tap **Add** — the COMANDOS AI Workspace icon appears on your home screen
5. Launch from home screen for the full native app experience

### 🤖 Android

1. Open COMANDOS AI Workspace in **Chrome** on your Android device
2. Tap the **three-dot menu** (⋮) → **"Add to Home screen"**
3. Tap **Add** — COMANDOS AI Workspace is now a native-feeling app on your device

---

## 📡 Mobile Access via Tailscale

Access COMANDOS AI Workspace from anywhere on your devices — no port forwarding, no VPN complexity.
...
4. **Open COMANDOS AI Workspace on your phone:**
...
**In the meantime:** Install COMANDOS AI Workspace as a PWA (see above) for a near-native desktop experience — it works great.
...
A fully managed cloud version of COMANDOS AI Workspace is in development:
...
The Docker setup runs the gateway automatically. Native dashboard is optional legacy compatibility.
...
| Themes | COMANDOS Dark / COMANDOS Light |
...
COMANDOS AI Workspace is free and open source. If it's saving you time and powering your workflow, consider supporting development:
...
<div align="center">
  <sub>Built with ⚡ by <a href="https://github.com/outsourc-e">@outsourc-e</a> and the COMANDOS AI Workspace community</sub>
</div>

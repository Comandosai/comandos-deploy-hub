#!/usr/bin/env bash
# COMANDOS Hermes Workspace — one-panel installer
#
# Usage:
...
REPO_URL="${REPO_URL:-https://github.com/Comandosai/comandos-workspace.git}"
INSTALL_DIR="${INSTALL_DIR:-$HOME/comandos-hermes-workspace}"
...
banner() {
  cat <<'EOF'
   ...
   ╭────────────────────────────────────────────╮
   │  COMANDOS HERMES — one-panel installer    │
   │  COMANDOS Workspace + Hermes gateway       │
   ╰────────────────────────────────────────────╯
...
if command -v hermes &>/dev/null; then
  ...
  if ! command -v hermes &>/dev/null; then
    ...
    yellow "    curl -fsSL https://raw.githubusercontent.com/Comandosai/comandos-workspace/main/install.sh | bash"
    exit 1
  fi
...
cyan "→ Cloning COMANDOS Workspace…"
...
ensure_env_key "$INSTALL_DIR/.env" "COMANDOS_SINGLE_PANEL" "1"
ensure_env_key "$INSTALL_DIR/.env" "HERMES_API_URL" "http://127.0.0.1:${GATEWAY_PORT}"
ensure_env_key "$INSTALL_DIR/.env" "HERMES_CLI_PATH" "$(command -v hermes)"
green "  .env ready ✓"
...
cyan "→ Building COMANDOS Workspace…"
pnpm_cmd build
green "  build ready ✓"
...
Native Hermes dashboard is not started by default. COMANDOS Workspace is the
main panel. Use "pnpm start:legacy-dashboard" only if you intentionally need
the old dashboard on port 9119.

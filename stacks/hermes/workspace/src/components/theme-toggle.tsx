...
const MODES: Array<{
  ...
  { value: 'system', icon: ComputerIcon, label: 'Система' },
  { value: 'light', icon: Sun01Icon, label: 'Светлая' },
  { value: 'dark', icon: Moon01Icon, label: 'Темная' },
]
...
export function ThemeToggle({ variant = 'pill' }: ThemeToggleProps) {
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        aria-label={`Тема: ${currentThemeLabel}. Переключить на ${isDark ? 'светлую' : 'темную'}`}
  ...
  return (
    <div
      ...
      role="group"
      aria-label={`Режим темы. Сейчас: ${currentThemeLabel}`}
    >
      {MODES.map((mode) => {
        ...
        return (
          <button
            ...
            aria-label={
              active ? `${mode.label} тема, выбрана` : `${mode.label} тема`
            }
            title={mode.label}
          >

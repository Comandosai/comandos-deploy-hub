...
export function AgentStreamPanel({ sessionKey, agentName, agentColor, onClose }: AgentStreamPanelProps) {
  ...
  const model = toStr(session?.model) || 'модель не указана'
  ...
  async function onPauseToggle() {
    ...
    try {
      ...
      toast(`${agentName}: ${nextPaused ? 'поставлен на паузу' : 'возобновлён'}`, { type: 'success' })
      void sessionsQuery.refetch()
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Не удалось изменить паузу', { type: 'error' })
    } finally {
  ...
  async function onKill() {
    ...
    try {
      ...
      toast(error instanceof Error ? error.message : 'Не удалось остановить агента', { type: 'error' })
    } finally {
  ...
  return (
    <>
      <button type="button" aria-label="Закрыть live-панель агента" className="fixed inset-0 z-40 bg-black/30" onClick={onClose} />
        ...
        <div className="flex h-full min-h-0 flex-col">
            ...
            <div className="flex items-start justify-between gap-3 px-4 py-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />В эфире</span>
                </div>
                <p className="mt-1 truncate text-xs text-neutral-500 dark:text-neutral-400">{model} · {sessionKey}</p>
              </div>
              <button type="button" onClick={onClose} className="rounded-lg px-2 py-1 text-sm text-neutral-500 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800" aria-label="Закрыть панель">×</button>
            </div>
            <div className="flex flex-wrap gap-2 px-4 pb-3">
              <span className="inline-flex items-center gap-1 rounded-lg bg-neutral-100 px-2 py-1 text-xs dark:bg-neutral-800"><span className="text-neutral-500 dark:text-neutral-400">Токены</span><span className="font-medium text-neutral-800 dark:text-neutral-200">{inputTokens.toLocaleString()} / {outputTokens.toLocaleString()}</span></span>
              <span className="inline-flex items-center gap-1 rounded-lg bg-neutral-100 px-2 py-1 text-xs dark:bg-neutral-800"><span className="text-neutral-500 dark:text-neutral-400">Стоимость</span><span className="font-medium text-neutral-800 dark:text-neutral-200">{formatCost(totalCost)}</span></span>
              <span className="inline-flex items-center gap-1 rounded-lg bg-neutral-100 px-2 py-1 text-xs dark:bg-neutral-800"><span className="text-neutral-500 dark:text-neutral-400">В работе</span><span className="font-medium text-neutral-800 dark:text-neutral-200">{formatRuntime(runtimeStart)}</span></span>
            </div>
          </div>
          <div ref={streamRef} className="flex-1 space-y-2 overflow-y-auto px-4 py-3">
            {messages.length === 0 ? <p className="text-xs text-neutral-500 dark:text-neutral-400">Жду сообщения...</p> : messages.map((message) => (
          ...
          <div className="sticky bottom-0 border-t border-neutral-200 bg-white/95 px-3 py-3 backdrop-blur dark:border-neutral-800 dark:bg-neutral-900/95">
            <div className="grid grid-cols-3 gap-2">
              <button type="button" onClick={() => setSteerOpen(true)} className="rounded-lg border border-neutral-200 px-2 py-2 text-xs font-medium text-neutral-800 hover:bg-neutral-100 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800">Направить</button>
              <div className="relative">
                {menuOpen ? <div className="absolute bottom-full left-0 mb-2 w-full rounded-lg border border-neutral-200 bg-white p-1 shadow-lg dark:border-neutral-700 dark:bg-neutral-900"><button type="button" onClick={() => void onPauseToggle()} disabled={pausePending} className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-neutral-700 hover:bg-neutral-100 disabled:opacity-60 dark:text-neutral-200 dark:hover:bg-neutral-800">{pausePending ? 'Обновляю...' : isPaused ? 'Продолжить' : 'Пауза'}</button><button type="button" onClick={() => void onKill()} disabled={killPending} className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-red-600 hover:bg-red-50 disabled:opacity-60 dark:text-red-300 dark:hover:bg-red-950/40">{killPending ? 'Останавливаю...' : 'Остановить'}</button></div> : null}
                <button type="button" onClick={() => setMenuOpen((open) => !open)} className="w-full rounded-lg border border-neutral-200 px-2 py-2 text-xs font-medium text-neutral-800 hover:bg-neutral-100 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800">Пауза/стоп</button>
              </div>
              <button type="button" onClick={() => { onClose(); void navigate({ to: '/chat/$sessionKey', params: { sessionKey } }) }} className="rounded-lg bg-accent-500 px-2 py-2 text-xs font-medium text-white hover:bg-accent-600">Открыть чат</button>
            </div>
          </div>
        </div>

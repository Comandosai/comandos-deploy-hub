...
export function GuardrailsModal({ open, agentName, agentId, sessionKey, onOpenChange }: GuardrailsModalProps) {
  ...
  function handleSave() {
    ...
    toast(`Ограничения обновлены для ${agentName}`, { type: 'success' })
    onOpenChange(false)
  }

  const stopRows: Array<{ checked: boolean; label: string; onToggle: () => void }> = [
    { checked: stopOnError, label: 'Остановить при ошибке агента', onToggle: () => setStopOnError((value) => !value) },
    { checked: stopOnIdle, label: 'Остановить при простое больше 5 минут', onToggle: () => setStopOnIdle((value) => !value) },
    { checked: stopOnTokenLimit, label: 'Остановить при достижении лимита токенов', onToggle: () => setStopOnTokenLimit((value) => !value) },
  ]
      ...
      <DialogContent className="w-[min(560px,92vw)]">
        <div className="space-y-4 p-5">
          <div className="space-y-1">
            <DialogTitle className="text-base">Ограничения: {agentName}</DialogTitle>
            <DialogDescription>Задайте правила поведения агента. Изменения сохраняются локально.</DialogDescription>
          </div>

          <section className="space-y-2">
            <p className="text-xs font-medium text-primary-700">Максимум токенов за запуск</p>
          ...
          <section className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-medium text-primary-700">Режим</p>
              <div className="inline-flex rounded-lg bg-neutral-100 dark:bg-neutral-800 p-1 gap-1">
                {(['allowlist', 'blocklist'] as const).map((mode) => (
                  ...
                  >
                    {mode === 'allowlist' ? 'Разрешённые' : 'Запрещённые'}
                  </button>
                ))}
              </div>
            </div>
            ...
            />
            <p className="text-[11px] text-primary-600">Введите имена инструментов по одному в строке. Оставьте пустым, чтобы разрешить все.</p>
          </section>

          <section className="space-y-1">
            <p className="text-xs font-medium text-primary-700">Автоостановка</p>
            {stopRows.map((row) => (
              <button
                key={row.label}
                type="button"
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <Button size="sm" onClick={handleSave} className="bg-accent-500 text-white hover:bg-accent-600">Сохранить</Button>
          </div>
        </div>
      </DialogContent>
    </DialogRoot>

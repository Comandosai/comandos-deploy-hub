...
export function MobileSetupModal({ isOpen, onClose }: MobileSetupModalProps) {
  ...
  const steps = [
    {
      title: 'Установите Tailscale на desktop',
      body: 'Установите Tailscale на машину, где запущен COMANDOS AI Workspace, затем войдите в аккаунт.',
    ...
    {
      title: 'Оставьте backend доступным',
      body: 'COMANDOS AI Workspace работает с OpenAI-compatible backend и на мобильном. Убедитесь, что workspace и backend доступны через Tailscale или локальную сеть.',
    ...
    {
      title: 'Установите Tailscale на телефон',
      body: 'Установите Tailscale на iOS или Android и войдите в тот же аккаунт.',
      showTailscaleIcon: true,
      action: (
    ...
    {
      title: 'Откройте COMANDOS AI Workspace на телефоне',
      body:
        networkUrl?.source === 'tailscale'
          ? 'Ваш Tailscale адрес. Откройте его в браузере телефона.'
          : networkUrl?.source === 'lan'
            ? 'Адрес в локальной сети. Телефон должен быть в том же Wi-Fi.'
            : 'Запустите Tailscale на этой машине, если нужен адрес вне localhost.',

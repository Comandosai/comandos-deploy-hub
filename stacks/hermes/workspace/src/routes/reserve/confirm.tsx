...
function ReserveConfirmRoute() {
  usePageTitle('Подтверждение брони HermesWorld')
  ...
  useEffect(() => {
    if (!token) {
      setState({
        status: 'error',
        message: 'Нет токена подтверждения. Откройте ссылку из письма ещё раз.',
      })
      return
    }
    ...
    fetch('/api/hermesworld/reservations/confirm', {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(payload.error || 'Не удалось подтвердить бронь')
        }
        setState({
          status: 'success',
          message: `Имя ${payload.reservation.desiredName} подтверждено для запуска.`,
        })
      })
      .catch((error: Error) => {
        setState({

...
function splitMetaList(value: string): Array<string> {
  return value
    .replace(/^\[|\]$/g, '')
    .split(',')
...
async function collectLocalSkillDirs(root: string): Promise<Array<string>> {
...
async function readLocalInstalledSkills(): Promise<Array<SkillSummary>> {
  const root = getSkillsDir()
  const dirs = await collectLocalSkillDirs(root)
  const skills = await Promise.all(
    dirs.map(async (dir) => {
      ...
      const name = readString(meta.name) || heading || id
      const description =
      ...
      return {
        ...
        name,
        description,
        author: readString(meta.author) || 'Local Hermes',
        triggers: splitMetaList(meta.triggers || ''),
        tags: splitMetaList(meta.tags || ''),
        homepage: readString(meta.homepage) || null,
        category: guessCategory({
          category: readString(meta.category) || path.basename(path.dirname(dir)),
          tags: splitMetaList(meta.tags || ''),
        }),
        icon: readString(meta.icon) || '✨',
        content: raw,
        fileCount: 1,
        sourcePath: dir,
        installed: true,
...
async function fetchClaudeSkills(): Promise<Array<SkillSummary>> {
  ...
  try {
    response = capabilities.dashboard.available
      ? await dashboardFetch('/api/skills')
      : await fetch(`${CLAUDE_API}/api/skills`, { headers })
  } catch (error) {
    if (COMANDOS_SINGLE_PANEL) return readLocalInstalledSkills()
    throw error
  }
  if (!response.ok) {
    if (COMANDOS_SINGLE_PANEL) return readLocalInstalledSkills()
  ...
  const remoteSkills = items
    .map((entry) => normalizeSkill(entry))
    .filter((entry): entry is SkillSummary => entry !== null)
  if (!COMANDOS_SINGLE_PANEL) return remoteSkills

  const merged = new Map<string, SkillSummary>()
  for (const skill of remoteSkills) merged.set(skill.id, skill)
  for (const skill of await readLocalInstalledSkills()) {

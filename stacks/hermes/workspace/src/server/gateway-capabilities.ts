 ...
 *
 * COMANDOS single-panel architecture:
 *   - Gateway (:8642 by default): /health, /v1/chat/completions, /v1/models
 *   - Workspace: the only visible panel; config and local skill inventory are
 *     read from $HERMES_HOME without requiring Hermes dashboard.
 *   - Dashboard (:9119 by default): optional legacy compatibility only.
 *
...
export const COMANDOS_SINGLE_PANEL = readBooleanEnv('COMANDOS_SINGLE_PANEL', true)
export const COMANDOS_REQUIRE_HERMES_DASHBOARD = readBooleanEnv(
  'COMANDOS_REQUIRE_HERMES_DASHBOARD',
  false,
)
let dashboardOverrideEnabled = Boolean(_initialOverrides.claudeDashboardUrl)
...
export function setDashboardUrl(input: string | null | undefined): string {
  ...
  if (normalized) {
    ...
    dashboardOverrideEnabled = false
  }
  writeOverrides(overrides)
  probePromise = null
  lastProbeAt = 0
...
export function getResolvedUrls(): {
  ...
  const overrides = readOverrides()
  const dashboardExplicit = Boolean(
    overrides.claudeDashboardUrl ||
      process.env.HERMES_DASHBOARD_URL ||
      process.env.CLAUDE_DASHBOARD_URL,
  )
  ...
  return { gateway: CLAUDE_API, dashboard, source }
}
...
function shouldUseHermesDashboard(): boolean {
  return (
    COMANDOS_REQUIRE_HERMES_DASHBOARD ||
    !COMANDOS_SINGLE_PANEL ||
    Boolean(
...
async function autoDetectDashboardUrl(): Promise<void> {
  if (!shouldUseHermesDashboard()) return
...
export async function probeGateway(options?: {
  ...
  probePromise = (async () => {
    ...
    capabilities = {
      ...
      enhancedChat,
      skills: dashboard.available || legacySkills || COMANDOS_SINGLE_PANEL,
      ...
      memory: true,
      config: dashboard.available || legacyConfig || COMANDOS_SINGLE_PANEL,

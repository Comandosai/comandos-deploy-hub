...
const DIALOG_LINES = [
  'Сегодня над HermesWorld идёт странный шум.',
  'Строители открыли новые панели, но замки хранилища отвечают только игроку, который знает старые маршруты.',
  'Найдите светящиеся точки на карте мира, проверьте инвентарь и вернитесь с целым сигилом основателя.',
]

const CHOICES: QuestChoice[] = [
  { id: 'ask-lore', label: 'Спросить о шуме', tone: 'Знания' },
  { id: 'promise', label: 'Пообещать вернуть сигил', tone: 'Героично' },
  { id: 'request-reward', label: 'Уточнить условия награды', tone: 'Практично' },
]

export function QuestDialogPanel({ npcName = 'Афина', npcTitle = 'Оракул Агоры', onAccept, onDecline }: QuestDialogPanelProps) {
  return (
    <section
      role="dialog"
      aria-label="Диалог задания"
      ...
      <div className="grid gap-0 md:grid-cols-[230px_1fr]">
        <aside className="border-b border-[#d9b35f]/25 bg-[radial-gradient(circle_at_50%_15%,rgba(251,191,36,.24),transparent_46%),linear-gradient(180deg,rgba(217,179,95,.16),rgba(0,0,0,.18))] p-5 md:border-b-0 md:border-r">
          <div className="mx-auto flex max-w-[190px] flex-col items-center text-center">
            ...
            <h2 className="mt-4 font-serif text-2xl font-black text-[#ffe7a3]">Диалог задания</h2>
        ...
        <div className="flex min-h-[430px] flex-col p-5">
          <div className="mb-3 flex items-center justify-between gap-3 border-b border-[#d9b35f]/20 pb-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#d9b35f]/70">Основное задание</p>
              <h3 className="font-serif text-xl font-black text-[#ffe7a3]">Сигнал основателя</h3>
            </div>
            <span className="rounded-full border border-emerald-300/35 bg-emerald-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-200">Доступно</span>
          </div>

          <div aria-label="Текст диалога" role="region" className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-[#d9b35f]/18 bg-black/28 p-4 shadow-inner">

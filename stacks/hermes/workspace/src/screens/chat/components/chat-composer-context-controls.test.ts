...
describe('ChatComposer context controls', () => {
  it('wires profile selection through the existing profile APIs', () => {
    ...
    expect(src).toContain('Профиль ${profileName} активирован')
  })
  ...
  it('surfaces workspace and reasoning controls next to the model picker', () => {
    ...
    expect(src).toContain('Настройки чата')
    ...
    expect(src).toContain('Усилие рассуждения')
    expect(src).toContain("['medium', 'Среднее']")
    expect(src).toContain("['high', 'Высокое']")
  })
})

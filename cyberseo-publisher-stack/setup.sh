...
GITHUB_STACK_PATH="cyberseo-publisher-stack"
...
sync_policy_layer() {
    local policy_src="mu-plugins/cyberseo-site-policy.php"
    local policy_dest="/var/www/html/wp-content/mu-plugins/cyberseo-site-policy.php"

    if [ ! -f "$policy_src" ]; then
        print_warning "Policy layer не найден локально: $policy_src"
        return 0
    fi

    docker exec comandos-wp mkdir -p /var/www/html/wp-content/mu-plugins
    docker cp "$policy_src" comandos-wp:"$policy_dest"
    docker exec comandos-wp chown www-data:www-data "$policy_dest"
    print_success "Policy layer синхронизирован: cyberseo-site-policy.php"
}
...
print_logo
print_header "${STACK_NAME} - INSTALLER v1.0.0"
...
GITHUB_BASE="https://raw.githubusercontent.com/Comandosai/comandos-deploy-hub/main/${GITHUB_STACK_PATH}"
...
FILES=(
    ...
    "js/customize-preview.js" "mu-plugins/cyberseo-site-policy.php"
)
...
if [ "$MODE" == "INSTALL" ]; then
    ...
    docker exec comandos-wp bash -c "wp plugin install wordpress-seo wp-graphql --activate --allow-root" || true

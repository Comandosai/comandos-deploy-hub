...
Если пользователь просит заполнить профиль сайта до развёртывания — обновить поля `site.*`, `topics.main_topic` и `prompts.*` в `cyberseo.project.yml`, ничего не разворачивать.

Если пользователь просит развернуть WordPress — поставить WordPress, загрузить брендовые файлы, вставить профиль сайта из `cyberseo.project.yml` и записать доступы в `.cyberseo.state.yml`.
...
- страна, город, основная тема и число тем;
- промпт писателя;
- промпт художника;

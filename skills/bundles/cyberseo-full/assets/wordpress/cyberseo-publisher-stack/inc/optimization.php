...
function comandos_render_managed_inline_images(string $content): string {
    if (is_admin()) {
        return $content;
    }

    return preg_replace_callback('/<figure\b[^>]*\bdata-media-id=(["\']?)(\d+)\1[^>]*>.*?<\/figure>/is', function($matches) {
        $media_id = isset($matches[2]) ? (int) $matches[2] : 0;
        if ($media_id <= 0) {
            return $matches[0];
        }

        $image_html = wp_get_attachment_image($media_id, 'large', false, [
            'class'         => 'article-inline-image wp-image-' . $media_id,
            'loading'       => 'lazy',
            'decoding'      => 'async',
        ...
        $caption_html = '';
        if (preg_match('/<figcaption\b[^>]*>(.*?)<\/figcaption>/is', $matches[0], $caption_match)) {
            $caption_html = '<figcaption style="font-size: 14px; color: #777; margin-top: 10px; font-style: italic; line-height: 1.4;">' . $caption_match[1] . '</figcaption>';
        }

        return '<figure class="wp-block-image aligncenter size-large comandos-inline-figure" data-media-id="' . $media_id . '" style="margin: 24px auto; text-align: center;">' . $image_html . $caption_html . '</figure>';
    }, $content);
}

add_filter('the_content', 'comandos_render_managed_inline_images', 90);

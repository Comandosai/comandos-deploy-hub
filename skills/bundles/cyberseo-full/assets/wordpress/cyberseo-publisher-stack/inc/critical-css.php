...
add_action('wp_head', function() {
    ...
    $aspect_ratio = get_theme_mod('global_img_aspect_ratio', '3 / 2');
    ...
    if (strlen($brand_hex) == 3) {
        $brand_r = hexdec(substr($brand_hex, 0, 1) . substr($brand_hex, 0, 1));
        $brand_g = hexdec(substr($brand_hex, 1, 1) . substr($brand_hex, 1, 1));
        $brand_b = hexdec(substr($brand_hex, 2, 1) . substr($brand_hex, 2, 1));
    } else {
        $brand_r = hexdec(substr($brand_hex, 0, 2));
        $brand_g = hexdec(substr($brand_hex, 2, 2));
        $brand_b = hexdec(substr($brand_hex, 4, 2));
    }
    $brand_brightness = (($brand_r * 299) + ($brand_g * 587) + ($brand_b * 114)) / 1000;
    $brand_button_text = ($brand_brightness > 155) ? '#111111' : '#ffffff';
    ...
    <style id="comandos-v22-final">
        ...
        :root {
            ...
            --slate-900: #0f172a;
            --slate-950: #020617;
            --primary-dark: color-mix(in srgb, var(--primary) 70%, #0f172a);
            ...
            --primary-glow-strong: color-mix(in srgb, var(--primary) 42%, transparent);
            --offer-soft-bg: color-mix(in srgb, var(--primary) 10%, white);
            --offer-soft-border: color-mix(in srgb, var(--primary) 28%, white);
            --offer-soft-accent: color-mix(in srgb, var(--primary) 52%, var(--slate-700));
            --offer-dark-bg: color-mix(in srgb, var(--primary) 12%, var(--slate-950));
            --offer-dark-badge: rgba(255, 255, 255, 0.92);
            --offer-glow-soft: color-mix(in srgb, var(--primary) 20%, transparent);
            --offer-glow-strong: color-mix(in srgb, var(--primary) 38%, transparent);
            --offer-button-bg: var(--primary);
            --offer-button-text: <?php echo esc_attr($brand_button_text); ?>;
        }

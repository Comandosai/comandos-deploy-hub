...
- загрузить workflow;
- загрузить внутренний `07_WF_CRM_Operato`, если в пакете используется CRM-оператор;
...
- `07_WF_CRM_Operato` считать внутренним workflow: импортировать и привязать, но не активировать как отдельный входящий сценарий. Он должен вызываться только из главного оркестратора как tool workflow.
...
- после импорта выставить режим активации:
  ...
  - все боевые workflow и `07_WF_CRM_Operato` оставить выключенными;
...
- workflow загружены;
- `07_WF_CRM_Operato` импортирован и привязан, если он есть в пакете;

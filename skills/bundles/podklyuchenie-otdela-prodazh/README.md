...
- не активировать боевые workflow `01-05` и внутренний `07_WF_CRM_Operato` до отдельной команды на боевой запуск отдела продаж;
...
- консультацию;
- передачу человеку;
- CRM-оператор `07_WF_CRM_Operato`, если он есть в пакете;
...
- проверить, что падения `01-07` будут уходить в workflow ошибок, если `07_WF_CRM_Operato` есть в пакете;
- подставить `Supabase`;
- подставить `Postgres`;
- подставить `MCP`;
...
- что боевые workflow `01-05` и внутренний `07_WF_CRM_Operato` выключены до отдельной команды.
- что у `01-07` указан актуальный `settings.errorWorkflow` на workflow ошибок, если `07_WF_CRM_Operato` есть в пакете.

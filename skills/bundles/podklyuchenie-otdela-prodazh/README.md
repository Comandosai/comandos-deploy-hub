...
- скачивает workflow закрытым архивом по `x_license_key`;
...
- не активировать боевые workflow `01-05` и `07_WF_CRM_Operator` до отдельной команды на боевой запуск отдела продаж;
- при боевом запуске и финальном тесте `07_WF_CRM_Operator` публиковать или активировать вместе с остальными рабочими workflow, если он есть в пакете;
...
Сначала скачай закрытый архив workflow по `x_license_key` из `DANNYE_DLYA_RAZVERTYVANIYA.md`.

Используй только этот источник:
`https://api.comandos.ai/skill-runtime/artifacts/sales-workflows/latest`

Скачивать нужно с заголовком:
`X-License-Key: <x_license_key>`

После скачивания распакуй архив во временную папку и импортируй все `*.json` из папки `workflows`.
...
Не брать workflow из открытого GitHub и не создавать их вручную.
...
- что боевые workflow `01-05` и `07_WF_CRM_Operator` выключены до отдельной команды.

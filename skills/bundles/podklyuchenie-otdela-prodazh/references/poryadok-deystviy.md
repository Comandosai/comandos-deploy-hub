...
Перед импортом нужно скачать закрытый архив workflow.

Источник:
- `https://api.comandos.ai/skill-runtime/artifacts/sales-workflows/latest`

Правило:
- брать `x_license_key` из `DANNYE_DLYA_RAZVERTYVANIYA.md`;
- передавать его в заголовке `X-License-Key`;
- распаковывать архив во временную папку;
- импортировать workflow только из распакованной папки `workflows`;
- если ключа нет или архив не скачался, не продолжать импорт.

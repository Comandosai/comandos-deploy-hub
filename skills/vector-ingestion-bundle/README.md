...
- документы лежат у пользователя локально;
- агент / runner запускается локально;
- локально идет запрос в `api.comandos.ai`;
- обратно локально приходят `chunks + embeddings`;
- потом локально же результат записывается в удаленный `Supabase`.

Важно:

- сервер с `Supabase` не является основным execution-host по умолчанию;
- на сервере с `Supabase` не нужно запускать весь ingestion flow, если у него нет доступа к `api.comandos.ai`;
- если execution-host без интернета, тогда используется только fallback:
  - получить JSON с `chunks + embeddings` на машине с интернетом;
  - потом импортировать этот JSON в `Supabase`.

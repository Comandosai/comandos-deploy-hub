...
- Особенно не создавать заранее `Postgres`, `Supabase`, `MCP Postgres`, `MCP AmoCRM`, `OpenAI`, `OpenRouter`, `Telegram Bot`.
...
- Конфигурация reverse proxy должна быть внутренне согласованной и соответствовать рабочему эталону `n8n.dev`: `Traefik` через `docker provider`, а публичные маршруты `n8n` через `docker labels`.
- Навык не должен оставлять в `n8n-stack-v2` ветку с `traefik_dynamic`, `file provider` и ручным копированием dynamic-конфига.
...
- проверить, что `Traefik` собран по модели `docker provider + labels`, без `traefik_dynamic` и без ручных dynamic-маршрутов;
...
- отсутствие лишних заранее созданных прикладных credentials;

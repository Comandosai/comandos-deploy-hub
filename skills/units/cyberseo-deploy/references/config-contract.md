...
Важно: комментарии в `cyberseo.project.yml` являются частью файла. При последующих обновлениях агент должен менять только значения и сохранять комментарии. Нельзя пересохранять весь YAML через обычный парсер, если из-за этого пропадают строки с `#`.

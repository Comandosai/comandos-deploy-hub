...
## Как обновлять `cyberseo.project.yml`

Комментарии в `cyberseo.project.yml` обязательны: это подсказки для пользователя. Нельзя переписывать файл через обычный YAML-dump, потому что он стирает комментарии.

Правило обновления:

- существующий файл править точечно, только нужные поля;
- использовать `apply_patch`, ручную замену строк или другой способ, который сохраняет комментарии;
- не пересобирать весь файл из объекта;
- перед массовым обновлением сделать копию;
- после обновления проверить, что комментарии `#` остались;
- если комментарии уже пропали, восстановить структуру из `references/config-contract.md` или `cyberseo-full/assets/cyberseo.project.yml` и перенести текущие значения.

...
```text
Сначала скачай локально только папку `skills/` из репозитория `https://github.com/Comandosai/comandos-deploy-hub`.
Не скачивай весь репозиторий без необходимости.
Stack-папки локально не тащи:
...
Дальше установи навык `Полный запуск отдела продаж` строго из локального пути `skills/polnyy-zapusk-otdela-prodazh`.
...
Для локальной установки скачивай только нужную skill-папку.

...
Агент сначала обязан проверить, что тестовый workflow `Проверка базы знаний` реально активирован. Если в текущей версии `n8n` используется публикация, он должен сделать `publish`; если используется старый режим, он должен включить `Active`. Только после этого запускать Telegram-тесты.
...
Перед тестами:
- найди workflow `Проверка базы знаний`;

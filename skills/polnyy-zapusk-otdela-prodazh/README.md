...
При первом разворачивании агент должен активировать только тестовый workflow `06_WF_Test`.
...
- активируй или опубликуй только `06_WF_Test`;
...
- workflow уведомлений об ошибках тоже не активируй на первом шаге, если пользователь отдельно не попросил;
- боевой запуск делать только после теста базы знаний и отдельной команды пользователя.

После завершения:
- обнови контекст проекта;
- покажи, какие workflow подключены;
- покажи, что активен только `06_WF_Test`, а боевые workflow выключены;
...
## Команда 10. Запустить боевые тесты отдела продаж
